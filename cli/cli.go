...
func (c *Cli) handleHead(cmd *Head) {
	if !c.requireClient() {
		return
	}

	req, err := c.buildQueryRequest(cmd.Table, cmd.Prefix, cmd.From, cmd.To, cmd.Limit, true)
	if err != nil {
		fmt.Printf("error: %v\n", err)
		return
	}
...
func (c *Cli) handleHelp() {
	...
	fmt.Println("                       Query earliest rows in range (default: last hour, limit 100)")
	fmt.Println("  query table=<t> [prefix=<p>] [from=<ts>] [to=<ts>] [limit=<n>]")
	fmt.Println("                       Query rows newest first (default: last hour, limit 100)")
	fmt.Println("  delete table=<t> [prefix=<p>] from=<ts> to=<ts>")
	fmt.Println("                       Delete rows in range (shows preview, requires confirmation)")
	fmt.Println()
	fmt.Println("  Time formats: unix timestamp, ISO 8601, or relative (-1h, -7d, -2w, now)")
	fmt.Println()
	fmt.Println("Backup / Restore:")
	fmt.Println("  backup [profile=<name>] [type=full|incr] [full_key=<key>]")
	fmt.Println("                       Backup to S3 (interactive profile picker if omitted)")
	fmt.Println("  restore [profile=<name>] key=<object_key>")
	fmt.Println("                       Restore from S3 backup")
	fmt.Println()
	fmt.Println("S3 profiles:")
	fmt.Println("  s3, s3 help          Show S3 help and usage")
	fmt.Println("  s3 list              List configured S3 profiles")
	fmt.Println("  s3 add [name=<name>] Add or update an S3 profile")
	fmt.Println("  s3 delete [name=<n>] Delete an S3 profile")
	fmt.Println()
	fmt.Println("Config:")
	fmt.Println("  config, config help  Show config help")
	fmt.Println("  config show          Show current server address")
	fmt.Println("  config set [addr=<a>] Set server address")
	fmt.Println("  config reset         Reset to default address")
}
